...
public class PriorityScheduler extends Scheduler {
	...
	protected class PriorityQueue extends ThreadQueue {
		...
		public void EnterAcquire(KThread thread) {
			Lib.assertTrue(Machine.interrupt().disabled());
			ThreadState t = getThreadState(thread);
			t.waitForAccess(this);
			t.acquire(this);
		}
	...
	protected class ThreadState {
		...
		void acquire(PriorityQueue priorityQ) {
			...
			if (priorityQ.lockingThread != null) {
				ThreadState t = getThreadState(priorityQ.lockingThread);
				if (t.acquired.remove(priorityQ)) {
					priorityQ.lockingThread = null;
					t.updateEffectivePriority();
				}
			}
